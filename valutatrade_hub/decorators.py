...
from valutatrade_hub.logging_config import LOG_CONFIG, log_action, log_error
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                elif "user" not in log_data and operation_name in [
                    "BUY",
                    "SELL",
                    "LOGIN",
                ]:
                ...
                if operation_name in ["BUY", "SELL"] and len(args) > 1:
                    log_data["currency_code"] = (
                        str(args[1]).upper() if args[1] else "unknown"
                    )
                    if len(args) > 2:
                        try:
