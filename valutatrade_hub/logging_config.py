...
def _format_log_string(action, **kwargs):
    ...
    for key in field_order:
        if key in kwargs and kwargs[key] is not None:
            ...
            if isinstance(value, str) and key in ['user', 'currency', 'base', 'result', 'error_type', 'error_message']:
    ...
    for key, value in kwargs.items():
        if key not in field_order and value is not None:
            if isinstance(value, str) and key in ['error_type', 'error_message', 'wallet_before', 'wallet_after']:
                fields.append(f"{key}='{value}'")
            else:
                fields.append(f"{key}={value}")
