...
def show_portfolio_command(session_data, base_currency: str = "USD"):
    ...
    if not portfolio_data:
        return {'success': False, 'message': f"Портфель для пользователя '{username}' не найден"}
...
def buy_command(session_data, currency: str, amount: float):
    ...
    if currency == 'USD':
        ...
        if rate_key not in rates:
            return {'success': False, 'message': f"Не удалось получить курс для {currency}→USD"}
...
def sell_command(session_data, currency: str, amount: float):
    ...
    try:
        ...
        return {
            'success': False,
            'message': f'У вас нет кошелька {currency}. Добавьте валюту: она создаётся автоматически при первой покупке'
            }
...
def get_rate_command(from_currency: str, to_currency: str):
    ...
    if from_currency == to_currency:
        return {'success': True, 'message': f"Курс {from_currency}→{to_currency}: 1.0000 (одна и та же валюта)"}
    ...
    if needs_update:
        ...
        if key in rates and isinstance(rates[key], dict):
            ...
            return {'success': False, 'message': f"Курс {from_currency}→{to_currency} недоступен"}
        '''
        # Обновляем кеш
        if key not in rates:
            rates[key] = {}
